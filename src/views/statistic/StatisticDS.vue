<template>
    <div class="flex flex-col min-h-screen bg-gradient-to-br from-emerald-50 to-white">
        ...
        <main class="flex-1 px-4 pb-6 flex flex-col gap-6">
            ...
            <section>
                <h3 class="text-lg font-semibold text-emerald-600 mb-4">Thống kê số lượng nam nữ theo trách vụ trong
                    từng ngành</h3>
                <div v-for="major in groupedResponsibilities" :key="major.majorId"
                    ...
                    <p class="text-gray-700 mb-2 text-center">Tổng số:
...
import { getRoleDSStats, getReligionStatsRoleDS, getResponsibilityMajorStatsDS } from "@/services/statisticService";
...
async function fetchDeaneries() {
    ...
    if (res.code === 200) {
        ...
        if (deaneries.value.length > 0) {
            selectedDeanery.value = deaneries.value[0].deaneryId; // mặc định chọn cái đầu tiên
            await fetchAllStats();
        }
    }
}
...
async function fetchStats(deaneryId: number) {
    const res = await getRoleDSStats(deaneryId);
    if (res.code === 200) {
...
async function fetchReligionStats(deaneryId: number) {
    const res = await getReligionStatsRoleDS(deaneryId);
    if (res.code === 200) {
...
async function fetchResponsibilities(deaneryId: number) {
    const res = await getResponsibilityMajorStatsDS(deaneryId);
    if (res.code === 200) {
